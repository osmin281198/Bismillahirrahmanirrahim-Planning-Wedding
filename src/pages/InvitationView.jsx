...
function FloatingMusic() {
  ...
  return (
    <div className="fixed bottom-24 right-4 z-50 flex flex-col items-end gap-2">
      ...
      {/* Volume slider */}
      {showVol && (
        <div className="bg-white rounded-2xl shadow-xl p-3 border border-sky-100 flex flex-col items-center gap-1">
          <span className="text-xs text-slate-400">🔊</span>
          <input type="range" min={0} max={1} step={0.01} value={volume}
            onChange={handleVolume}
            className="h-20 accent-sky-500 cursor-pointer"
      ...
      <div className="flex gap-2">
        <button onClick={(e) => { e.stopPropagation(); setShowVol(!showVol); }}
          className="w-10 h-10 rounded-full flex items-center justify-center shadow-lg border border-sky-100 bg-white text-sky-500 text-sm">
          🎚
        </button>
        <button onClick={togglePlay}
          className="w-12 h-12 rounded-full flex items-center justify-center shadow-xl text-white text-lg transition hover:scale-110 active:scale-95"
          style={{ background: "linear-gradient(135deg, #0284C7, #38BDF8)" }}>
          {playing ? "⏸" : "▶"}
        </button>
      </div>

      {/* Animasi not balok saat playing */}
      {playing && (
        <div className="flex gap-0.5 justify-center mt-1">
          {[1,2,3,4].map((i) => (
            <div key={i} className="w-1 rounded-full bg-sky-400"
              style={{
                height: "12px",
...
export default function InvitationView() {
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {[...Array(20)].map((_, i) => (
            <div key={i} className="absolute rounded-full opacity-10 animate-pulse"
              style={{ background: C.lightBlue, width: Math.random()*80+20+"px", height: Math.random()*80+20+"px",
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", background: C.cloudBlue }} className="pb-20">
          ...
          <div className="bg-white rounded-3xl overflow-hidden shadow-sm border border-sky-100">
            ...
            <div className="p-6 text-center">
              ...
              <a href={`https://maps.google.com/?q=${encodeURIComponent(settings.wedding_location || "Bandung")}`}
                ...
                className="inline-block mt-3 px-6 py-2.5 rounded-full text-white text-sm font-medium hover:opacity-90"
      ...
      <section id="ucapan" className="py-16 px-6" style={{ background: C.cloudBlue }}>
        ...
        <div className="max-w-sm mx-auto bg-white rounded-3xl p-6 shadow-sm border border-sky-100 mb-6">
          ...
          <textarea className="w-full border border-slate-200 p-3 rounded-xl text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-sky-400"
            placeholder="Ucapan & doa..." rows={3} value={rsvpMsg} onChange={(e) => setRsvpMsg(e.target.value)} />
          <button onClick={submitRsvp} className="w-full py-3 rounded-xl text-white text-sm font-semibold hover:opacity-90" style={{ background: gradLight }}>
            Kirim Ucapan
          </button>
      ...
      {/* Bottom Nav */}
      <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-sky-100 shadow-xl"
